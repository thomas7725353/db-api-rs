...
pub async fn handle_api(
    ...
    let config = match config {
        ...
        _ => return (StatusCode::NOT_FOUND, json!({"error": "API not found or offline"}).to_string()).into_response(),
    };

    // 2. Get DataSource
    let ds_id = match config.datasource_id {
        Some(id) => id,
        None => return (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": "DataSource ID missing"}).to_string()).into_response(),
    };

    let ds = match DataSource::select_by_id(&state.metadata_db, ds_id).await {
        Ok(Some(ds)) => ds,
        _ => return (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": "DataSource not found"}).to_string()).into_response(),
    };

    // 3. Get/Create RBatis pool
    let rb = match state.pool_manager.get_or_create(&ds).await {
        Ok(rb) => rb,
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": format!("Failed to connect to datasource: {}", e)}).to_string()).into_response(),
    };
    ...
    let dialect = match ds.db_type.as_deref().unwrap_or("").to_lowercase().as_str() {
        ...
        _ => return (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": "Unsupported database type"}).to_string()).into_response(),
    };

    let sql = config.sql.as_deref().unwrap_or("");
    let (transformed_sql, param_names) = match SqlTransformer::transform(sql, dialect) {
        Ok(res) => res,
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": format!("SQL transformation failed: {}", e)}).to_string()).into_response(),
    };
    ...
    let param_values = match SqlTransformer::extract_params(&param_names, &all_params) {
        Ok(vals) => vals,
        Err(e) => return (StatusCode::BAD_REQUEST, json!({"error": e.to_string()}).to_string()).into_response(),
    };
    ...
    match rb.exec_decode::<JsonValue>(&transformed_sql, rbs_values).await {
        Ok(result) => (StatusCode::OK, result.to_string()).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, json!({"error": format!("SQL execution failed: {}", e)}).to_string()).into_response(),
    }
}
